...
async function confirmDonor (data){
    ...
    try {
        const response = await fetch(`/patients/store-confirm${isEdit[0] ? `?patient=${isEdit[1].id}` : ''}`, {
            method: 'POST',
            headers: {
        ...
        if (!response.ok) {
            ...
            Swal.fire({
                icon: "success",
                title: `Patient ${isEdit[0] ? 'Updated' : 'Registered'} Successfuly`,
