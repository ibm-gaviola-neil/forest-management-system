...
document.getElementById('add-donor-form').addEventListener('submit', async function(event) {
    ...
    try {
        const response = await fetch(`/donors/${donor_id.value}/edit-confirm`, {
            method: 'POST',
            headers: {
                "X-CSRF-TOKEN": document
                    .querySelector('meta[name="csrf-token"]')
