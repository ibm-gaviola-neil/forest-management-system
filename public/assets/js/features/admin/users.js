...
const loginForm = document.getElementById('add-user-form');
...
if (profileEditForm) {
    profileEditForm.addEventListener('submit', async (e) =>{
        ...
        try {
            ...
            redirectModal('Profile updated successfully!', '/admin/users/view/' + id.value);
        } catch (error) {
    ...
    return `
      <tr class="hover:bg-gray-50 transition-colors duration-200">
        ...
        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
            <a href="/admin/users/view/${data.id}" class="text-green-600 hover:text-green-900 mr-3">View</a>
        </td>
      </tr>
    `;
}
...
if(loginForm){
    loginForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            showSuccessAlert('New user created successfully successfully. Redirecting', 'success');
            
            // Create a success confirmation popup
            redirectModal(
                'New user created successfully successfully',
                '/admin/users'
            );
